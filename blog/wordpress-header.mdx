...
If we have a full screen height hero image we need to take into account the menu header and also the admin bar when logged in. Using the [mobile chin code](/components/hero#mobile-chin) we can use a custom property to set the height.
