...
The initial state of the image needs to have zero opacity, plus the transition settings will be added. Add the javascript to an `onload` attribute to change the classes to affect the opacity.

The background colour needs to be added to the container element of the image.

```html
<img 
  class="transition-opacity duration-300 opacity-0 ease-in"
  onload="
    javascript: this.classList.add('opacity-40');
    this.classList.remove('opacity-0');
  "
/>
```

Note, it seems to work best by adding the `onload` code to the image tag and therefore the code should always run. If the javascript is added after the tag it is possible the image has already loaded, especially on localhost when the image is cached.

Any overlay text to be animated in will need an id attached and set the classes.

```html
<h1 
  id="hero-text" 
  class="opacity-0 duration-300 transition delay-300 transform translate-y-4"
>
  Welcome
</h1>
```

Additional javascript is added to the `onload` attribute to affect a fade in and up.
...
<img 
  ...
  onload="
    javascript: this.classList.add('opacity-40');
    this.classList.remove('opacity-0');
    document.getElementById('hero-text').classList.remove('opacity-0', 'translate-y-4');
  "
/>
```

And finally, we should add a `preload` to the header to ensure the hero image loads quickly. This needs to include all the srcset image sizes so that the correct size will be preloaded.

The `href` can be removed so unsupported brwosers do not download an unneeded image. [credit](https://www.stefanjudis.com/today-i-learned/how-to-preload-responsive-images-with-imagesizes-and-imagesrcset/)

```php
<?php 
  if( is_page_template( 'page-templates/home.php' ) ):

    $hero = get_field('hero_desktop_image');
?>
  <link
    rel="preload" 
    as="image" 
    href="<?php echo $hero['sizes']['full_2560']; ?>"
    imagesrcset="
...
If using `<picture>` for art direction of different breakpoints then we need to write the preload links for each breakpoint using the `media` attribute. [credit](https://web.dev/preload-responsive-images/) 

```
<?php 
  if( is_page_template( 'page-templates/fome.php' ) ):
    $hero = get_field('hero_desktop_image');
